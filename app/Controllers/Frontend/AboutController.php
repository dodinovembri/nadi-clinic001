...
class AboutController extends BaseController
{
    public function index($trial_name = null)
    {
        ...
        $data['about'] = $about->where('client_id', $client_id)->where('status', 1)->get()->getFirstRow();     
        // faqs
        $faq = new FaqModel();
        $data['faqs'] = $faq->where('client_id', $client_id)->where('status', 1)->get()->getResult();
        // benefit
        $benefit = new BenefitModel();
        $data['benefits'] = $benefit->where('client_id', $client_id)->where('status', 1)->get()->getResult(); 
        // team
        $team = new TeamModel();
        $data['teams'] = $team->where('client_id', $client_id)->where('status', 1)->get()->getResult();                     
